...
const csvFile = fs.readFileSync('./data/wef.csv', 'utf8')

Papa.parse(csvFile, {
    header: true,
    complete: function(results) {
        results.data.forEach(row => {
            if (row.address && row.maxClaimable) {
                data[row.address] = parseFloat(row.maxClaimable)
            }
        })
    }
})
...
export async function wefAirdropChecker() {
