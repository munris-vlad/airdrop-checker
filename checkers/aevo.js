...
let p
let csvWriter
let wallets = readWallets('./addresses/aevo.txt')
...
async function checkAirdrop(wallet, proxy = null) {
    ...
    while (!isFetched) {
        await axios.get(`https://api-ribbon.vercel.app/api/aevo/check-eligibility?address=${wallet}`, config).then(async response => {
            stats[wallet].airdrop = response.data.airdrop.toString()
            isFetched = true
        }).catch(e => {
...
export async function aevoAirdropChecker() {
