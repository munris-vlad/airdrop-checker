...
const csvFile = fs.readFileSync('./data/task_master_prize_results.csv', 'utf8')

Papa.parse(csvFile, {
    header: true,
    complete: function(results) {
        results.data.forEach(row => {
            if (row.wallet_address && row.lxp_point) {
                data[row.wallet_address] = parseFloat(row.lxp_point)
            }
        })
    }
})
...
export async function lineaParkAirdropChecker() {
